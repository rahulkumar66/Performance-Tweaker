  ...
  public static List<String> run(String shell, String[] commands, String[] environment,
    ...
    try {
      ...
      StreamGobbler STDERR =
          new StreamGobbler(shellUpper + "*", process.getErrorStream(), wantSTDERR ? res : null);
  ...
  public static class SU {
    ...
    public static synchronized String version(boolean internal) {
      ...
      if (suVersion[idx] == null) {
        ...
        List<String> ret = Shell.run(internal ? "su -V" : "su -v", new String[] {}, null, false);
  ...
  public static class Interactive {
    ...
    private synchronized boolean open() {
      ...
      try {
        ...
        STDOUT =
            new StreamGobbler(shell.toUpperCase(Locale.ENGLISH) + "-", process.getInputStream(),
                new OnLineListener() {
                  @Override public void onLine(String line) {
                    synchronized (Interactive.this) {
                      ...
                      if (line.startsWith(command.marker)) {
                        try {
                          lastExitCode =
                              Integer.valueOf(line.substring(command.marker.length() + 1), 10);
                        } catch (Exception e) {
                        }
        ...
        STDERR =
            new StreamGobbler(shell.toUpperCase(Locale.ENGLISH) + "*", process.getErrorStream(),
                new OnLineListener() {
                  @Override public void onLine(String line) {
                    synchronized (Interactive.this) {
                      if (command == null) {
