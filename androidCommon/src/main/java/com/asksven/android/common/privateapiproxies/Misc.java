...
public class Misc extends StatElement implements Comparable<Misc>, Serializable {
        ...
        try {
          ...
          if ((this.getName().equals(myRef.getName())) && (this.getuid() == myRef.getuid())) {
            ...
            if (this.m_timeOn > this.m_timeRunning) {
              Log.i(TAG,
                  "Fixed rounding difference: " + this.m_timeOn + " -> " + this.m_timeRunning);
  ...
  @Override public String toString() {
    return "Misc [m_name="
        + m_name
        + ", m_timeOn="
        + formatDuration(getTimeOn())
        + ", m_timeRunning="
        + formatDuration(getTimeRunning())
        + "]";
  }
  ...
  public String getVals() {
    ...
    return m_name
        + " "
        + this.formatDuration(getTimeOn())
        + " ("
        + getTimeOn() / 1000
        + " s)"
        + " in "
        + this.formatDuration(getTimeRunning())
        + " ("
        + getTimeRunning() / 1000
        + " s)"
        + " Ratio: "
        + formatRatio(getTimeOn(), getTimeRunning());
  }
