 ...
 * @author sven
 */
public class NativeKernelWakelock extends StatElement
    implements Comparable<NativeKernelWakelock>, Serializable {
   ...
   */
  @SerializedName("details") private String m_details;
   ...
   * the expire count
   */
  @SerializedName("expire_count") private int m_expireCount;

  /**
   * the wake count
   */
  @SerializedName("wake_count") private int m_wakeCount;

  /**
   * the active_since time
   */
  @SerializedName("active_since") private long m_activeSince;

  /**
   * the total_time
   */
  @SerializedName("total_time") private long m_ttlTime;

  /**
   * the sleep time
   */
  @SerializedName("sleep_time") private long m_sleepTime;

  /**
   * the max time
   */
  @SerializedName("max_time") private long m_maxTime;

  /**
   * the last change
   */
  @SerializedName("last_change") private long m_lastChange;
   ...
   */
  public NativeKernelWakelock(String name, String details, int count, int expire_count,
      int wake_count, long active_since, long total_time, long sleep_time, long max_time,
      long last_change, long time) {
  ...
  public NativeKernelWakelock clone() {
    NativeKernelWakelock clone =
        new NativeKernelWakelock(m_name, m_details, m_count, m_expireCount, m_wakeCount,
            m_activeSince, m_ttlTime, m_sleepTime, m_maxTime, m_lastChange, getTotal());
    clone.m_icon = m_icon;
        ...
        try {
          ...
          if ((this.getName().equals(myRef.getName())) && (this.getuid() == myRef.getuid())) {
            ...
            if ((m_count < 0) || (m_sleepTime < 0) || (this.getTotal() < 0)) {
              Log.e(TAG, "substractFromRef generated negative values ("
                  + this.toString()
                  + " - "
                  + myRef.toString()
                  + ")");
